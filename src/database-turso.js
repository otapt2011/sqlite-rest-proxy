...
// Deep convert an object: replace any "avatar" or "cover_image_data" base64 string with Uint8Array for insertion
function prepareForDb(obj) {
  if (!obj || typeof obj !== 'object') return obj;
  for (const key in obj) {
    if ((key === 'avatar' || key === 'cover_image_data') && typeof obj[key] === 'string' && obj[key].length > 0) {
...
// Deep convert result rows: replace any Uint8Array avatar or cover_image_data with base64 string for JSON
function prepareForResponse(rows) {
  ...
  if (Array.isArray(rows)) {
    return rows.map(row => {
      ...
      if (newRow.cover_image_data && newRow.cover_image_data instanceof Uint8Array) {
        newRow.cover_image_data = uint8ArrayToBase64(newRow.cover_image_data);
      }
      return newRow;
    });
    ...
    if (rows.cover_image_data && rows.cover_image_data instanceof Uint8Array) {
      rows.cover_image_data = uint8ArrayToBase64(rows.cover_image_data);
    }
  }
  return rows;
}
...
export async function insert(table, data) {
  const db = await getDatabase();
  const preparedData = prepareForDb({ ...data }); // Convert base64 avatar/cover_image_data to Uint8Array
...
// New: execute raw SQL with parameters (POST /api/query will use this)
export async function executeQueryWithParams(sql, params = []) {
  const db = await getDatabase();
